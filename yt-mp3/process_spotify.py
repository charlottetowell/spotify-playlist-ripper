...
def process_spotify_playlists(spotify_data):        
    ...
    if not isinstance(spotify_data, list):
        print("Invalid input file structure. Expected a list of playlists.")
        return []
    if not all(isinstance(playlist, dict) for playlist in spotify_data):
        print("Invalid input file structure. Each playlist should be a dictionary.")
        return []
    if not all('tracks' in playlist and isinstance(playlist['tracks'], list) for playlist in spotify_data):
        print("Invalid input file structure. Each playlist should have a 'tracks' key with a list of tracks.")
        return []
    if not all('name' in track and 'artists' in track and 'id' in track for playlist in spotify_data for track in playlist['tracks']):
        print("Invalid input file structure. Each track should have 'name', 'id', and 'artists' keys.")
        return []
